...
const Profile = () => {
	...
	const handleLogout = async (e: React.MouseEvent) => {
		...
		try {
			...
			// Даже если произошла ошибка, перенаправляем на страницу логина
			navigate('/login');
		}
	};
	...
	// Не показываем ошибку, если пользователь есть - это позволяет работать с профилем
	// даже если загрузка деталей не удалась (например, при ошибке 500 на бэкенде)
	if (error && !user) {
		return (
