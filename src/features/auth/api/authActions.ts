...
import { loginShopAccount, logoutUserGet as logoutApi, fetchShopProfile, updateShopProfile } from '@shared/utils/api';
...
export const loginUser = createAsyncThunk(
	...
	async (
		...
		try {
			...
			if (!data.user) {
				console.log('No user data in login response, will be fetched by Autologin');
				return {
					ok: true,
					user: null,
...
export const logoutUserGet = createAsyncThunk(
	...
	async (_, { rejectWithValue, dispatch }) => {
		try {
			...
			console.log('Cleared all localStorage items and reset Redux state after GET logout');
			...
			console.log('Cleared localStorage and reset Redux state after GET logout error');
			
			return rejectWithValue(err instanceof Error ? err.message : 'Неизвестная ошибка выхода');
		}
	}
);
...
export const updateUser = createAsyncThunk(
	...
	async (
		...
		try {
			...
			console.log('Update user data:', { email, nickname, password: password ? '***' : 'none' });
